...
- [x] 博客首页（分类筛选 + 最新/阅读量排序 + 明确分页）
- [x] 响应式文章卡片（整卡可点击、独立分类/标签链接、可选封面和标签数量收敛）
- [x] 文章详情页（Markdown 渲染 + 代码高亮）
- [x] 博客前台按分类 / 标签筛选文章
- [x] 文章搜索（支持标题、摘要、分类和标签关键词）
- [x] 响应式顶部导航（当前栏目状态、移动端搜索和分类菜单）
- [x] 博客前台公开 API（无需登录）
- [x] 文章浏览量统计（访问详情页自动 +1）
- [x] 侧边栏热门文章（固定浏览量 Top 3，Redis 为空时从数据库自动重建）
- [x] 评论系统（支持楼中楼回复、根评论分页、树形嵌套展示）
- [x] 作者发现页（关键词、热门/最近活跃排序、分页）
- [x] 作者公开主页（简介、领域、统计、安全外链、已发布文章）
- [x] 文章卡片与详情可进入作者主页，公开作者摘要批量组装
...
|------|------|
| `sys_user` | 用户表 |
| `author_application` | 作者申请表 |
| `author_profile` | 审核通过后的作者公开资料 |
| `blog_article` | 文章表 |
| `blog_category` | 分类表 |
| `blog_tag` | 标签表 |
