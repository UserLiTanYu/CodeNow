...
categories=$(curl --fail --silent --show-error -H "$auth_header" "$BACKEND_URL/api/categories")
tags=$(curl --fail --silent --show-error -H "$auth_header" "$BACKEND_URL/api/tags")
...
comments=$(curl --fail --silent --show-error -H "$auth_header" "$BACKEND_URL/api/comments?pageNum=1&pageSize=100&articleId=$article_id")
...
upload_response=$(curl --fail --silent --show-error -H "$auth_header" -F "file=@$image_file;type=image/png" "$BACKEND_URL/api/upload/image")
